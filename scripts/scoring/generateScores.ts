...
if (isMain) {
    const [
          venueDir = "data/processed/venues",
          evidenceDir = "data/processed/evidence",
          outputDir = "data/processed/scores",
        ] = process.argv.slice(2);

  const venueFiles = fs
      .readdirSync(path.resolve(venueDir))
      .filter((name) => name.endsWith(".canonical.json"));

  const venues = venueFiles.map((file) =>
        readJson<CanonicalVenue>(path.resolve(venueDir, file))
                                  );
  ...
  const evidenceByVenue: Record<string, EvidenceRecord[]> = {};
    venues.forEach((venue) => {
          ...
          evidenceByVenue[venue.id] = fs.existsSync(evidencePath)
            ? readJson<EvidenceRecord[]>(evidencePath)
                  : [];
    });
