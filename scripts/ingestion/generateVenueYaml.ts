...
import {
    validateCanonicalVenue,
    validateScoreRecord,
    validateYamlPublishability,
...
export function generateVenueYaml(
...
if (isMain) {
    const [
          canonicalPath,
          scorePath,
          outputRoot = "content/venues",
          editorialPath,
        ] = process.argv.slice(2);

  if (!canonicalPath || !scorePath) {
        throw new Error(
                "Usage: node scripts/ingestion/generateVenueYaml.ts <canonical.json> <score.json> [output-root] [editorial.json]"
              );
  }

  const venue = JSON.parse(
        fs.readFileSync(path.resolve(canonicalPath), "utf8")
      ) as CanonicalVenue;
    const score = JSON.parse(
          fs.readFileSync(path.resolve(scorePath), "utf8")
        ) as ScoreRecord;
    const editorial = editorialPath
      ? (JSON.parse(fs.readFileSync(path.resolve(editorialPath), "utf8")) as Record<string, unknown>)
          : {};
  ...
  const scoreValidation = validateScoreRecord(score);
    if (!scoreValidation.valid) {
          throw new Error(`Score invalid: ${scoreValidation.errors.join(", ")}`);
    }
